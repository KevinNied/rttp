...
export function AppShell({
  ...
  return (
    <div className="min-h-dvh bg-app text-foreground selection:bg-primary selection:text-primary-foreground">
      ...
      {!vistaPrevia && !workoutImmersive && (
        ...
        >
          ...
          {!sidebarCompact && (
            <div className="mt-10 px-3 text-[10px] font-medium uppercase tracking-[0.18em] text-content-muted">
              {esEntrenador ? "Workspace" : "Entrenamiento"}
            </div>
          )}
          <nav
            aria-label="Navegación principal"
            className={cn("space-y-1.5", sidebarCompact ? "mt-6" : "mt-3")}
          >
            {navegacion.map((item) => {
                ...
                <button
                  ...
                  className={cn(
                    ...
                    (
                      ...
                      ? "bg-primary/12 text-content-primary"
                      : "text-content-secondary hover:bg-indigo-300/[0.07] hover:text-content-primary",
                  )}
                >
                  ...
                  {!sidebarCompact && (
                    <span className="min-w-0 flex-1 text-left">
                      ...
                      <span className="mt-1 block truncate text-[10px] leading-tight text-content-muted transition-colors group-hover:text-content-secondary">
                        {item.description}
                      </span>
                    </span>
                  )}
          ...
          >
            ...
            <Button
              ...
              title="Cerrar sesión"
              className={cn(
                "mt-2 h-9 rounded-xl border border-white/[0.06] bg-white/[0.03] px-3 text-xs text-content-secondary hover:bg-red-300/10 hover:text-red-700 dark:hover:text-red-100",
                sidebarCompact
      ...
      {!vistaPrevia && !workoutImmersive && (
        <nav
          aria-label="Navegación principal"
          className="pointer-events-none fixed inset-x-0 bottom-0 z-40 flex justify-center pb-[max(0.75rem,env(safe-area-inset-bottom))] pl-[max(0.75rem,env(safe-area-inset-left))] pr-[max(0.75rem,env(safe-area-inset-right))] lg:hidden"
        >
